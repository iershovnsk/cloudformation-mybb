...
zones; currently I only created two per tier.

- **Multiple environments**: I assumed this is a production-only infrastructure, therefore the
automation template contain no mechanism for conditionally creating stacks for other environment
types (e.g. DEV, QA or PRELIVE). Such rules could be added.
...
- **Remove Public IPs in PublicSubnets**: Currently I set "MapPublicIpOnLaunch" to true on public subnets for a quick way to debug the launch configuration; this should not be required in production.
...
- The automation template (along with this documentation) are revision-controlled on a public **GitHub repository**; this repository is also used (for simplicity) to host the Bash installation script for MyBB 1.8.6. It would be preferable that this would be turned into a new AMI instance.
...
the HTTPS:// endpoint for better security/privacy (currently only the security groups have been configured to allow access on port 443 - i.e. HTTPS).
